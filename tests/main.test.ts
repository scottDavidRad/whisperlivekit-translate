...
test('full app routes SDK audio, session controls, captions, and AI lifecycle', { timeout: 30_000 }, async t => {
  ...
  try {
    ...
    await t.test('starts automatically after the acknowledged task and displays eight native-size caption lines', async () => {
      ...
      const firstPage = Array.from({ length: 8 }, (_, i) => `Caption ${i + 1}.`).join(' ')
      socket.transcript(firstPage)
      await until(() => lensText().includes('Caption 8.'), 'expanded captions reach the glasses')
      const steadyPage = lensText()
      assert.equal(steadyPage.split('\n').length, 8)
      assert.match(steadyPage, /Caption 1\./)
      socket.message({ lines: [{ speaker: 1, text: firstPage }], buffer_transcription: 'Rapidly changing draft words.' })
      await sleep(180)
      assert.equal(lensText(), steadyPage, 'Interim wording must not move or revise the glasses page')
      assert.match(phoneText(), /Rapidly changing draft/, 'The phone still shows live wording')
      socket.transcript(Array.from({ length: 12 }, (_, i) => `Caption ${i + 1}.`).join(' '))
      await sleep(800)
      assert.equal(lensText(), steadyPage, 'Overflow waits for reading time instead of scrolling rows')
      await until(() => lensText().includes('Caption 12.'), 'overflow advances as a whole page', 3000)
    ...
    await t.test('Conversate requests transcription and rejects old speech or AI during a mode switch', async () => {
      ...
      socket.transcript(Array.from({ length: 7 }, (_, i) => `Caption ${i + 1}.`).join(' '))
      await until(() => lensText().includes('Caption 5.'), 'five-line captions follow the mode change')
      assert.equal(lensText().split('\n').length, 5)
      assert.match(lensText(), /Caption 1\./)
      assert.doesNotMatch(lensText(), /Caption 6\./, 'Unread overflow must not push existing rows away')
